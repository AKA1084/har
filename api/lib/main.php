<?php
    ...
    function customer(string $id, array $options = []): array{
        ...
        if($id === "all") $sql = "select ${fields} from customers_record";
        else{ 

            $id = degenerateId($id);
            $sql = "select ${fields} from customers_record where binary id = ${id}";
    ...
    function transferMoney(string $sid, string $rid, float $amount): array{
        ...
        function updateBalance(int $id, float $balance){
            ...
            global $conn;
            $conn->cud("update customers_record set balance = ${balance} where binary id = ${id}");
        ...
        if($sbal >= $amount){
            ...
            $res = $conn->cud("insert into transactions_record (sid, rid, amount, date_time) values (${sid}, ${rid}, ${amount}, '${datetime}')");
    ...
    function transaction(string $id): array{
        ...
        $res = $conn->r("select sid, rid, amount, date_time from transactions_record where binary sid = ${id} OR rid = ${id}");
